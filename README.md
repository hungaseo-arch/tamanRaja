...
2. **차월 핸디 조정 (`next_hc`)** — 참석자 기준
   - **기본 규칙**
     - 전월과 **같은 조**(2개월 연속): 1등조 → `app_hc − 1`, 2등조 → `app_hc + 1`
     - 조가 **바뀐 경우**: `next_hc = std_hc` (기준 핸디로 리셋)
     - 미참석: `app_hc` 유지
   - **기준 핸디 재적용(리셋) 월** — 1월(`-01`) 및 연중 기준 핸디를 새로 적용한 달(예: **2026-07**): 직전 달과 조를 비교할 수 없으므로 **조 변경 리셋을 적용하지 않고** 1등조 → `app_hc − 1`, 2등조 → `app_hc + 1` 만 적용한다. 조 변경에 따른 리셋은 그 **다음 달부터**(예: 2026-08~) 적용된다.
     - 리셋 월 판별: 직전 달 대비 `std_hc`가 바뀐 달.
3. **시상**
   - **Winner** — 최저 Net Score
